...
from lucid.cognition.input.cue.encoder import expand_cue_aliases, expand_cue_weights
...
def tracebank_from_checkpoint(path: str | Path) -> list[DmfTraceRecord]:
    ...
    store_path = resolve_checkpoint(path) / "tracebank.json"
...
class DynamicMemoryField:
    ...
    def reindex_trace(self, idx: int) -> None:
        ...
        for key in record.cue_affinities:
            if key:
                index_keys.update(expand_cue_aliases(key))
        self._trace_index_keys[idx] = index_keys
        for alias in index_keys:
            self._cue_index.setdefault(alias, set()).add(idx)
    ...
    def _score_trace(
        ...
        score = 0.0
        matched = 0
        for affinity_key, affinity in record.cue_affinities.items():
            if affinity <= 0:
                continue
            w = max(
                (cue_weights.get(alias, 0.0) for alias in expand_cue_aliases(affinity_key)),
                default=0.0,
            )
            if w > 0:
                score += w * affinity
    ...
    def run(self, dmf_input: DmfInput) -> DmfOutput:
        cue_weights = expand_cue_weights(self.cue_key_weights(dmf_input.cue_cloud))
