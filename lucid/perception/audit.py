"""Perception audit logs for LLM-backed evidence extraction."""
...
_AUDIT_VERSION = "perception-audit-v1"
...
class PerceptionAuditLogger:
    """Write one readable JSON file per perception run."""

    def __init__(self, base_dir: str | Path = "audit/perception") -> None:
    ...
    def write(
        ...
        *,
        ...
        body = {
            ...
            "timestamp": utc_now(),
            "success": success,
            "human_summary": self._summary(inp, config, attempts, graph, error),
            "input": input_summary(inp),
        ...
        path.write_text(json.dumps(_jsonable(body), indent=2, ensure_ascii=False), encoding="utf-8")
        return path

    def _config_summary(self, config: Any) -> dict[str, Any]:
        return {
