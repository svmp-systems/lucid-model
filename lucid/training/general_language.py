    ...
    *,
    ...
    if not (root / "manifest.json").is_file():
        raise FileNotFoundError(
            f"checkpoint not found at {root} — run lucid.training.quantum_articles first or pass --checkpoint"
        )
    ...
    episodes = generate("chat_social", episode_count, seed=seed)
    write_episodes(episodes, pack_dir / "chat_social.jsonl")
    write_episodes(episodes, pack_dir / "all.jsonl")

    checkpoint_arg = str(checkpoint)
    module_runs: dict[str, Any] = {}
    if run_module_train:
        for module in ("lucidity", "decoder", "dmf", "cue_encoder"):
            cmd = [
                sys.executable,
                "-m",
                "lucid.training.cli",
    ...
    archived = register_checkpoint(
        name=name,
        path=root,
        label="quantum articles + general language v0.0.1",
