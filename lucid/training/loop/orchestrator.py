...
class TrainingAuditLogger:
    ...
    def __init__(self, base_dir: str | Path | None = None) -> None:
        from lucid.runtime.paths import DEFAULT_AUDIT_TRAINING_RUNS, resolve_train_path

        self.base_dir = resolve_train_path(base_dir or DEFAULT_AUDIT_TRAINING_RUNS)
...
class _GoldEpisodeValidatorWrapper:
    def __init__(self) -> None:
        from lucid.training.validate.gold import GoldEpisodeValidator, episode_from_training

        self._inner = GoldEpisodeValidator()
        self._episode_from_training = episode_from_training

    def evaluate(self, run_log: RunLog, episode: TrainingEpisode) -> ValidationResult:
        return self._inner.evaluate_run_log(run_log, self._episode_from_training(episode))
...
class ValidatorFactory:
    def __init__(self) -> None:
        self._validators = {
            ...
            "gold_episode": _GoldEpisodeValidatorWrapper(),
        }
...
class PromotionManager:
    ...
    def promote(
        ...
        hook = live_state.get("checkpoint_hook")
        if hook is not None:
            from lucid.training.validate.gold import episode_from_training

            training_episode = episode_store[target_episode_id]
            hook.on_promote(patch, episode_from_training(training_episode))
