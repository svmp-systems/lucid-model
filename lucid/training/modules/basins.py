...
class BasinsTrainer(ModuleTrainer):
    ...
    def train(
        ...
        for target in targets:
            ...
            signature[family] = min(1.0, max(float(signature.get(family, 0.0)), target["confidence"]))
            semantic = record.setdefault("semantic_signature", {})
            semantic[family] = min(1.0, max(float(semantic.get(family, 0.0)), target["confidence"]))
