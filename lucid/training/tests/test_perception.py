...
def test_infer_unit_positions_restores_sentence_order() -> None:
    from lucid.cognition.input.perception import infer_unit_positions
    from lucid.ir.perception import CandidateUnit, PerceptualEvidenceGraph

    graph = PerceptualEvidenceGraph(
        candidate_units=[
            CandidateUnit(unit_id="u_found", surface="found"),
            CandidateUnit(unit_id="u_money", surface="money"),
            CandidateUnit(unit_id="u_kayaking", surface="kayaking"),
            CandidateUnit(unit_id="u_placed", surface="placed"),
            CandidateUnit(unit_id="u_bank", surface="bank"),
        ]
    )
    sentence = "I found money while kayaking and placed it in the bank."
    infer_unit_positions(graph, sentence)
    positions = [int(u.position_or_time) for u in graph.candidate_units]
    assert positions == sorted(positions)
    assert graph.candidate_units[0].position_or_time == str(sentence.index("found"))
    assert graph.candidate_units[-1].position_or_time == str(sentence.index("bank"))
