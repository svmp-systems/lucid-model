...
def test_universal_cli_basins_smoke_writes_audit(
    ...
    assert lucid_main(["basins", "--audit-dir", str(audit_dir)]) == 0

    output = json.loads(capsys.readouterr().out)
    run_dirs = list((audit_dir / "runs").iterdir())
    manifest = json.loads((run_dirs[0] / "manifest.json").read_text(encoding="utf-8"))
    stage = json.loads((run_dirs[0] / "basins.json").read_text(encoding="utf-8"))
    assert output["candidate_basin_states"]
    assert len(run_dirs) == 1
    assert manifest["stages"][0]["stage_name"] == "basins"
    assert stage["summary"]["headline"].startswith(
        f"{len(output['candidate_basin_states'])} basins"
    )
